---
title: Microsoft Copilot Returns as a Sidebar in Windows 11 - TechPowerUp
url: https://news.google.?oc=5
published: ''
date_basis: harvested
harvested: '2026-05-25'
author: ''
feed_url: http://www.google.com/news/rss
---
Microsoft Copilot Returns as a Sidebar in Windows 11 TechPowerUp Microsoft lets users exile floating Copilot button after interface rage The Register Microsoft's big Copilot rollback continues as Office now lets you move its button to the ribbon XDA Microsoft admits the floating Copilot button in Word, Excel and PowerPoint was a mistake, lets you hide it after backlash Windows Latest Windows 11 now lets you remove Microsoft Copilot app with Group Policy or Registry, as it tries to win back users Windows Latest
